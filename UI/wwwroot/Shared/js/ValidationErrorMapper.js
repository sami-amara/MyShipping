...
const ValidationErrorMapper = (function () {
    ...
    function findInputForKey(formEl, key) {
        ...
        for (const sel of candidates) {
            try { const el = formEl.querySelector(sel); if (el) return el; } catch (e) { }
        }
    ...
    function showInlineMessage($form, inputRef, message) {
        ...
        try {
            ...
            if (parent) {
                ...
                // Update text
                if (errorSpan.textContent !== String(message)) errorSpan.textContent = message;
                ...
                try {
                    ...
                    if (!icon) {
                        ...
                        try {
                            ...
                            if (!cs || cs.position === 'static' || cs.position === '') parent.style.position = 'relative';
                        } catch (e) { }
                        try {
                            parent.classList.add('has-field-error');
                        } catch (e) { }
                ...
                try {
                    ...
                    try {
                        const parent = this.closest('.form-group') || this.parentElement;
                            ...
                            our.forEach(el => { try { el.remove(); } catch (e) { } });
                            ...
                            try { parent.classList.remove('has-field-error'); } catch (e) { }
                        }
                    } catch (e) { }

                    // Remove class and aria-invalid
                    try { this.classList.remove('field-error'); } catch (e) { }
                    try { this.removeAttribute('aria-invalid'); } catch (e) { }
                    ...
                    try {
                        ...
                        if (described.length) $this.attr('aria-describedby', described.join(' ')); else $this.removeAttr('aria-describedby');
                    } catch (e) { }
                    ...
                    try {
                        ...
                        if (nm) {
                            ...
                            $rv.each((i, el) => { try { const $el = $(el); $el.text(''); } catch (e) { } });
                        }
                    } catch (e) { }

                } catch (e) { }
                try { $(this).off('.ValidationErrorMapper'); } catch (e) { }
            };

            // remove previous then attach
            try { $input.off('.ValidationErrorMapper'); } catch (e) { }
            $input.on('input.ValidationErrorMapper change.ValidationErrorMapper', clearHandler);
    ...
    function mapErrorsToFields(errorMap, formElement) {
                    ...
                    msgs.forEach(msg => {
                        try {
                            ...
                            try {
                                ...
                                if ($fs && $fs.length) step = parseInt($fs.attr('data-step'), 10);
                            } catch (e) { }
                            mappedEntries.push({ $el: $el, dom: dom, msg: msg, step: Number.isInteger(step) ? step : null });
                        } catch (e) { }
                    });
                    anyMapped = true;
                    if (!firstMappedDom) firstMappedDom = dom;
                    break;
                }
            }
        });
        ...
        let targetStep = null;
        mappedEntries.forEach(me => { if (me.step !== null) { if (targetStep === null || me.step < targetStep) targetStep = me.step; } });
        if (targetStep === null && firstMappedDom) {
            try { const $fs = $(firstMappedDom).closest('fieldset[data-step]'); if ($fs && $fs.length) targetStep = parseInt($fs.attr('data-step'), 10); } catch (e) { }
        }

        try {
            if (targetStep !== null) {
                const anyEntry = mappedEntries.find(me => me.step === targetStep) || { dom: firstMappedDom };
                if (anyEntry && anyEntry.dom) revealStepForField($form, anyEntry.dom);
        ...
        mappedEntries.forEach(me => {
            try {
                ...
                if (belongs) showInlineMessage($form, me.$el, me.msg);
            } catch (e) { }
        });
        ...
        try {
            ...
            if ($first && $first.length) {
                ...
                if (dom) {
                    if (window.ScrollHelper?.scrollToCenter) { try { ScrollHelper.scrollToCenter(dom); } catch (e) { } }
                    else { try { dom.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (e) { } }
                    try { dom.classList.add('field-pulse'); setTimeout(() => dom.classList.remove('field-pulse'), 900); } catch (e) { }
                }
            }
        } catch (e) { }
