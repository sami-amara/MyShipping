...
const StripePayment = (function () {
     ...
     */
    async function processPayment(shipmentId, amount) {
        ...
        try {
            ...
            const createData = await ApiClient.postJson('api/Payment/CreateStripeIntent', {
            ...
            const captureData = await ApiClient.postJson('api/Payment/CaptureStripe', {
                shipmentId: shipmentId,
            ...
            if (!captureData || !captureData.success) {
                throw new Error(captureData?.error || captureData?.message || 'Failed to capture payment');
            }
            ...
            if (window.showAlert && typeof showAlert.Success === 'function') {
                showAlert.Success(
                    window.AppResourceAlerts?.paymentSuccessTitle || 'Payment Successful',
                    window.AppResourceAlerts?.paymentSuccessMessage || 'Your payment has been processed successfully.'
                );
            } else {
            ...
            if (window.showAlert && typeof showAlert.Error === 'function') {
                showAlert.Error(
                    window.AppResourceAlerts?.paymentFailedTitle || 'Payment Failed',
                    error.message
                );
            } else {
 ...
 * @param {string} shipmentId - Shipment GUID
 */
async function openStripePaymentModal(shipmentId) {
    try {
        ...
        form.onsubmit = async (e) => {
            e.preventDefault();
            await StripePayment.processPayment(shipmentId, amount);
        };
