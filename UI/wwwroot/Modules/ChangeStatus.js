/* eslint-disable no-undef */
    ...
    (function () {
        ...
        $(document).ready(function () {
            ...
            try { console.debug('ChangeStatus.js initializing. form=', form ? form.getAttribute('id') : null); } catch (e) { }
                ...
                if (window.ShipmentService && typeof ShipmentService.changeStatus === 'function') {
                    try {
                        ...
                        ShipmentService.changeStatus(id, payload, { redirect: true, redirectUrl: '/admin/Shipments/List' })
                            ...
                            .catch(err => {
                                cleanup();
                                const message = err && (err.message || err.responseJSON?.message || err.responseJSON?.Message) || (alerts.updateFailed || 'Failed to update shipment.');
                                notifyError(message);
                            });
                        return;
                    } catch (e) {
            ...
            // ═══════════════════════════════════════════════════════════════
            const shipmentId = document.querySelector('[name="Id"]')?.value;
            ...
            const approveAction = function (btnElem) {
                const id = document.querySelector('[name="Id"]')?.value || document.querySelector('#Id')?.value || shipmentId || null;
                if (!id) {
                    console.error('Approve: no shipment id found');
                    if (window.showAlert && typeof showAlert.Error === 'function') showAlert.Error((alerts.deleteFailedTitle || 'Error'), (alerts.missingShipmentId || 'Missing shipment id'));
                    return Promise.reject(new Error(alerts.missingShipmentId || 'Missing shipment id'));
                }
                try {
                    // determine approved numeric state
                    const approvedState = (typeof Business !== 'undefined' && Business.Services?.Shipment?.ShipmentStatusEnum?.Approved)
            ...
            if (btnApprove) {
                ...
                btnApprove.addEventListener('click', function (evt) {
                    ...
                    approveAction(this).catch(err => {
                        const message = err && (err.message || err.responseJSON?.message || err.responseJSON?.Message) || (alerts.approveFailed || 'Failed to approve shipment.');
                        if (window.showAlert && typeof showAlert.Error === 'function') showAlert.Error((alerts.deleteFailedTitle || 'Error'), message);
                        else alert(message);
                    });
                });
            }
            ...
            if (carrierSelectEl) {
                const callFill = (sel) => {
                    ...
                    if (window.ManagePageControlls && typeof ManagePageControlls.fillCarrierDropdown === 'function') {
                        try { ManagePageControlls.fillCarrierDropdown('#deliveryManId', currentCarrier); } catch (e) { console.warn('fillCarrierDropdown failed', e); }
                        return;
                    }
                            ...
                            CarriersService.GetAll(function (response) {
                                ...
                                if (Array.isArray(response)) list = response;
                                    ...
                                    for (const k in response) {
                                        if (Object.prototype.hasOwnProperty.call(response, k) && Array.isArray(response[k])) { list = response[k]; break; }
                                    }
                                }
                                if (!list) return;
                                ...
                                try {
                                    sel.innerHTML = '';
                                    const opt = document.createElement('option'); opt.value = ''; opt.textContent = 'Select Carrier'; sel.appendChild(opt);
                                    for (const it of list) {
                                        const val = it.id ?? it.Id ?? it.carrierId ?? it.CarrierId;
                                        const txt = it.carrierName ?? it.CarrierName ?? it.name ?? it.Name;
                                        if (!val) continue;
                                        const o = document.createElement('option'); o.value = val; o.textContent = txt || String(val); sel.appendChild(o);
                                    }
                                    if (currentCarrier) sel.value = currentCarrier;
                                } catch (e) { console.warn('Failed to populate carriers fallback', e); }
                            }, function (err) {
                                console.warn('CarriersService.GetAll failed', err);
                            });
                        } catch (e) { console.warn('CarriersService.GetAll threw', e); }
                        return;
                    }
            ...
            if (btnMarkShipped) {
                ...
                btnMarkShipped.addEventListener('click', function () {
                    const id = document.querySelector('[name="Id"]')?.value || shipmentId || null;
                    console.debug('ChangeStatus: mark shipped clicked for shipmentId=', id);
                    if (!id) {
                        if (window.showAlert && typeof showAlert.Error === 'function') showAlert.Error((alerts.deleteFailedTitle || 'Error'), (alerts.missingShipmentId || 'Missing shipment id'));
                        else alert(alerts.missingShipmentId || 'Missing shipment id');
                        return;
                    }

                    if (window.ShipmentService && typeof ShipmentService.adminActions === 'function') {
                        const shippedState = (typeof Business !== 'undefined' && Business.Services?.Shipment?.ShipmentStatusEnum?.Shipped) ? Business.Services.Shipment.ShipmentStatusEnum.Shipped : 5;
                        ShipmentService.adminActions(id, { action: 'changeStatus', targetState: shippedState, button: this, redirect: true, redirectUrl: '/admin/Shipments/List?shipped=1&shippedId=' + encodeURIComponent(id) })
                            .catch(err => {
                                const message = err && (err.message || err.responseJSON?.message || err.responseJSON?.Message) || (alerts.shippedFailed || 'Failed to mark shipment as shipped.');
                                if (window.showAlert && typeof showAlert.Error === 'function') showAlert.Error((alerts.deleteFailedTitle || 'Error'), message);
                                else alert(message);
                            });
                        return;
