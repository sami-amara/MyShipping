...
(function () {
    ...
    function attachInlineError(inputEl, message, form) {
        ...
        try {
            ...
            if ($group.length) {
                if ($group.find('.field-error-message').length === 0)
                    $group.append(`<span class="field-error-message text-danger" style="font-size:0.95em;">${message}</span>`);
                else
                    $group.find('.field-error-message').first().text(message);
            }
            else {
                if ($input.next('.field-error-message').length === 0)
                    $input.after(`<span class="field-error-message text-danger" style="font-size:0.95em;">${message}</span>`);
                else
                    $input.next('.field-error-message').first().text(message);
            }
    ...
    function mapServerErrorsImpl(raw, form) {
        ...
        try {
            ...
            console.error('mapServerErrors: mapping failed', ex);
        }
        if (hasFieldErrors) {
            try {
                if (typeof (FormValidator === null || FormValidator === void 0 ? void 0 : FormValidator.showStep) === 'function' && firstErrorStep !== null)
                    FormValidator.showStep(firstErrorStep);
                if (firstErrorInput && ((_a = window.ScrollHelper) === null || _a === void 0 ? void 0 : _a.scrollToCenter))
    ...
    function mapServerErrors(raw, form) {
        try {
            if (window.ValidationErrorMapper && typeof ValidationErrorMapper.mapErrorsToFields === 'function') {
                try {
                    return !!ValidationErrorMapper.mapErrorsToFields(raw, form);
                }
                catch (e) {
                    console.warn('ValidationErrorMapper.mapErrorsToFields threw', e);
                }
            }
        }
        catch (ex) {
    ...
    function extractFirstMessage(errors) {
        ...
        try {
            ...
            console.warn('extractFirstMessage failed', e);
        }
        return null;
    }
    function formatCurrency(v, currency = 'USD', locale = (navigator.language || 'en-US')) {
        if (v === null || v === undefined || v === '')
            return '-';
        try {
            return new Intl.NumberFormat(locale, { style: 'currency', currency }).format(Number(v));
        }
        catch (_a) {
            return String(v);
        }
