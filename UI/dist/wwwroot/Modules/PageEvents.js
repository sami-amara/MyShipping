        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (window.ApiClient && typeof window.ApiClient.getAccessToken === 'function') {
                    ...
                    yield window.ApiClient.getAccessToken();
            ...
            try {
                const defaultsJson = (_b = (_a = document.querySelector('#shipment-defaults-json')) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim();
                if (defaultsJson) {
            ...
            const senderCountryId = (_c = defaults.senderCountryId) !== null && _c !== void 0 ? _c : defaults.countryId;
            const senderCityId = (_d = defaults.senderCityId) !== null && _d !== void 0 ? _d : defaults.cityId;
            const receiverCountryId = (_e = defaults.receiverCountryId) !== null && _e !== void 0 ? _e : defaults.countryId;
            const receiverCityId = (_f = defaults.receiverCityId) !== null && _f !== void 0 ? _f : defaults.cityId;
            // Now initialize all dropdowns (token is cached and ready)
            // Countries first, then dependent cities to avoid race conditions
            yield ManagePageControlls.fillCountryDropdownAsync($('select[name="SenderCountry"]'), senderCountryId);
            yield ManagePageControlls.fillCountryDropdownAsync($('select[name="ReceiverCountry"]'), receiverCountryId);
            // Load cities only when a country is known; each party uses its own country+city pair
            if (senderCountryId) {
                yield ManagePageControlls.fillCityDropdownAsync($('select[name="SenderCity"]'), senderCountryId, senderCityId);
            }
            if (receiverCountryId) {
                yield ManagePageControlls.fillCityDropdownAsync($('select[name="ReceiverCity"]'), receiverCountryId, receiverCityId);
            }
            // For Shipping Types - pass default type ID
            yield ManagePageControlls.fillShippingTypesDropdownAsync('select[name="ShippingTypes"]', defaults.typeId);
            // For Shipping Packaging - pass default package ID
            yield ManagePageControlls.fillShippingPackgingDropdownAsync('select[name="ShippingPackging"]', defaults.packageId);
            ...
            if (document.querySelector('#deliveryManId')) {
                yield ManagePageControlls.fillCarrierDropdownAsync('#deliveryManId', defaults.carrierId);
            }
            ...
            if ($settingsCountry.length) {
                ...
                if (currentCountryId) {
                    ...
                    yield ManagePageControlls.fillCityDropdownAsync($settingsCity, currentCountryId, currentCityId);
                }
            }
