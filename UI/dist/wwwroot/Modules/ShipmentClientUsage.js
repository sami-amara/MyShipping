...
(function () {
    ...
    function _num(paged, ...keys) {
        for (const k of keys) {
            const v = paged === null || paged === void 0 ? void 0 : paged[k];
    ...
    function loadPage(page = 1) {
        ...
        if (window.ShipmentApiClient && typeof ShipmentApiClient.getPaged === 'function') {
            ShipmentApiClient.getPaged(page, pageSize, sortBy, sortDir)
                ...
                if (!paged) {
                    if (tbody)
                        tbody.innerHTML = '<tr><td colspan="8" class="text-center">No data returned</td></tr>';
                    if (window.ShipmentsPager)
                        window.ShipmentsPager.renderPagination({ Page: page, PageSize: pageSize, TotalPages: 1, TotalCount: 0 }, tableSelector, loadPage);
                    return;
                }
                const items = paged.Items || paged.items || paged.data || paged.Data || [];
                const itemsArr = Array.isArray(items) ? items : (items ? [items] : []);
                const normalizedPage = (_a = _num(paged, 'Page', 'page')) !== null && _a !== void 0 ? _a : page;
                const normalizedPageSize = (_b = _num(paged, 'PageSize', 'pageSize')) !== null && _b !== void 0 ? _b : pageSize;
                ...
                if (window.ShipmentsPager)
                    window.ShipmentsPager.renderPagination({ Page: 1, PageSize: pageSize, TotalPages: 1, TotalCount: 0 }, tableSelector, loadPage);
            });
        }
            ...
            if (tbody)
                tbody.innerHTML = '<tr><td colspan="8" class="text-center text-danger">Client not available</td></tr>';
        }
    }
...
//////    });
//////})();
//# sourceMappingURL=ShipmentClientUsage.js.map