(function ($) {
    ...
    if ($('textarea[name=code-editable]').length) {
        const editableCodeMirror = CodeMirror.fromTextArea(document.getElementById('code-editable'), {
            mode: 'javascript',
            theme: 'ambiance',
            lineNumbers: true
        });
    ...
    if ($('#code-readonly').length) {
        const readOnlyCodeMirror = CodeMirror.fromTextArea(document.getElementById('code-readonly'), {
            mode: 'javascript',
            theme: 'ambiance',
            lineNumbers: true,
            readOnly: 'nocursor'
    ...
    if ($('#cm-js-mode').length) {
        const cm = CodeMirror(document.getElementById('cm-js-mode'), {
            mode: 'javascript',
            lineNumbers: true
        });
    }
    ...
    if ($('.multiple-codes').length) {
        var code_type = '';
        const editorTextarea = $('.multiple-codes');
    ...
    if ($('#ace_json').length) {
        $(function () {
            const editor = ace.edit('ace_json');
            editor.setTheme('ace/theme/monokai');
    ...
    if ($('#ace_ruby').length) {
        $(function () {
            const editor = ace.edit('ace_ruby');
            editor.setTheme('ace/theme/monokai');
    ...
    if ($('#ace_coffee').length) {
        $(function () {
            const editor = ace.edit('ace_coffee');
            editor.setTheme('ace/theme/monokai');
